          ...
          for (var i=0, length=data.length; i < length; i++) {
            ...
            if ( live_data.due_date && (target.hasClass('unknown') || target.hasClass('page')) ) {
              ...
              if ( dom_item.data('request-url') ) {
                var link = $(
                  "<a rel='nofollow' class='btn btn-default btn-xs request-button' title='Opens in new tab' target='_blank' href='" + dom_item.data('request-url') + "'>Request <span class='sr-only'>(opens in new tab)</span></a>"
                );
