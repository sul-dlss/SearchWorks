...
const mapBlacklightQueryParamsToForm = (queryParams, defaultData) => {
  ...
  if (queryParams.range) {
    formData.filterFields = Object.entries(queryParams.range).map(([field, range]) => {
      return {
        id: currentId++,
        field: field,
  ...
  return (
    <fieldset className="d-flex flex-row mb-3 gap-3 align-items-center">
      ...
      <FormControl sx={{ minWidth: 190 }} size="small">
        <InputLabel id={`search-field-operator-${id}`} className="visually-hidden">Search operator</InputLabel>
      ...
      <label className="visually-hidden" htmlFor={`search-field-${id}`}>Search term</label>
      <TextField id={`search-field-${id}`} fullWidth size="small" value={value} onChange={(e) => dispatch({ type: 'updateSearchField', id: id, data: { value: e.target.value } })} />
...
const RangeFilterField = ({ id, field, values }) => {
  ...
  return (
    <>
      ...
      <TextField id={`range-field-${id}-begin`} name={values[0] && values[0].length > 0 && `range[${field}][begin]`} value={values[0] || ''} onChange={(event) => { dispatch({ type: 'updateFilterField', id: id, data: { values: [event.target.value, values[1]] } })}} size="small"></TextField>
      <span className="lh-lg">&ndash;</span>
      <label className="visually-hidden" htmlFor={`range-field-${id}-end`}>To year</label>
      <TextField id={`range-field-${id}-end`} name={values[1] && values[1].length > 0 && `range[${field}][end]`} value={values[1] || ''} onChange={(event) => { dispatch({ type: 'updateFilterField', id: id, data: { values: [values[0], event.target.value] } })}} size="small"></TextField>
    </>
  );
}
      ...
      <Autocomplete
        ...
        renderValue={(value, getItemProps) => (
          value.map((option, index) => {
            ...
            return (
              <Chip key={option} label={valueLabels[option] || option} {...itemProps} />
            );
          })
        )}
        renderInput={(params) => (
