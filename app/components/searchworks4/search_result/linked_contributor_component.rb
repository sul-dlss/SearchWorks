...
module Searchworks4
  module SearchResult
    ...
    class LinkedContributorComponent < ViewComponent::Base
      ...
      # Render as list item with linked name and post_text (e.g. role).
      # If there was an original language name, link it first followed by
      # transliterated version in parentheses as a separate link
      def call
        helpers.tag.li do
          if vern?
            safe_join([vern_contributor_link, " (", contributor_link, ")", post_text])
          else
            safe_join([contributor_link, post_text])
          end
        end
      end
