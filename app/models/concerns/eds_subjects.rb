  ...
  class Subject
    ...
    class Term < Struct.new(:fieldCode, :searchTerm, :label)
      ...
      def to_html
        ...
        # TODO: we don't have a view context for `link_to` or `articles_path`
        "<a href=\"/articles?q=#{CGI.escape(searchTerm)}&amp;search_field=#{field}\">#{CGI.escapeHTML(label)}</a>"
      end
    end
    ...
    def self.from(string_or_array)
      ...
      xml = case string_or_array
            when String
              convert_simple_string_to_xml(string_or_array)
            when Array
              string_or_array.map { |s| convert_simple_string_to_xml(s) }.join('<br/>')
            end
      from_xml(xml)
    end
