class NearbyOnShelf
  def self.around_item(item, search_service:, per: 24, **kwargs)
    ...
    before = NearbyOnShelf.reverse(search_service: search_service).items(item.reverse_shelfkey, per: per / 2, **kwargs)
    current_and_after = NearbyOnShelf.forward(search_service: search_service).items(item.shelfkey, per: per / 2, incl: true, **kwargs)
