...
describe "Databases Access Point", feature: true, "data-integration": true do
  ...
  it "should filter by letter" do
    ...
    expect(page).to have_css("h3.index_title", text: "19th century British Library newspapers [electronic resource].")
    expect(page).not_to have_css("h3.index_title", text: "Handbook of Latin American studies")

    within(".database-prefix .pagination") do
      click_link "H"
    end

    expect(page).not_to have_css("h3.index_title", text: "19th century British Library newspapers [electronic resource].")
    expect(page).to have_css("h3.index_title", text: "Handbook of Latin American studies")
  end
end
