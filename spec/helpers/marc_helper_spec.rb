...
RSpec.describe MarcHelper do
  ...
  describe '#get_uniform_title' do
    ...
    it 'selects search_title search_field if the MARC field tag is 130' do
      doc = SolrDocument.new(uniform_title_display_struct: [{
                               fields: [{
                                 uniform_title_tag: '130',
                                 field: { pre_text: 'xyz', link_text: 'Instrumental music Selections', post_text: '[print/digital]' }, vernacular: { vern: '' }
                               }]
                             }])
      title = get_uniform_title(doc)
